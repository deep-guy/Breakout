...
brickgreen=pygame.image.load("iceblock.jpg")
...
speedblock=pygame.image.load("blackbrick.jpg")
...
def fastblock(blx,bly,bls,fb,i):
    ...
    if fb[i]:
        pygame.draw.rect(gameDisplay,blue,[blx,bly,bls,block_size])
        brick2=pygame.transform.scale(speedblock,(bls,block_size))
        gameDisplay.blit(brick2,(blx,bly))
...
def fspeedcontrol(speedtoggle):
    ...
    for i in range(2):
        if fc[i]<500 and fc[i]>0:
            ...
            speedtoggle=2
    if speedtoggle==1:
        gameDisplay.blit(fireball,(lead_x,lead_y))        
        if lead_x_change<0:
            lead_x_change=-8
        else:
            lead_x_change=8
